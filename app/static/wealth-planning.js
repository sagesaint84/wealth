...
(() => {
  ...
  function renderHistory() {
    ...
    if(!records.length) { summaryBox.replaceChildren(); plot.replaceChildren(); renderHistoryDetails(details, records, allRecords); return; }
    ...
    const bars=records.map((r,i)=>{if(!i || !deltas[i])return '';const height=Math.abs(deltas[i])/maxDelta*42, y=deltas[i]>0?barBaseline-height:barBaseline;return `<rect class="wealth-history-bar ${deltas[i]>0?'is-positive':'is-negative'}" data-history-date="${esc(r.date)}" x="${points[i][0]-barWidth/2}" y="${y}" width="${barWidth}" height="${height}" rx="2"><title>${esc(r.date)} · 전 기록 대비 변화 ${deltas[i]>0?'+':''}${won(deltas[i])}</title></rect>`;}).join('');
    ...
    renderHistoryDetails(details, records, allRecords);
    const setHistoryActive=date=>{const root=historyPanel;root.classList.toggle('has-history-hover',Boolean(date));root.querySelectorAll('[data-history-date]').forEach(node=>node.classList.toggle('is-history-active',node.dataset.historyDate===date));};
    plot.querySelectorAll('[data-history-date]').forEach(node=>{node.addEventListener('pointerenter',()=>setHistoryActive(node.dataset.historyDate));node.addEventListener('pointerleave',()=>setHistoryActive(null));});
    details.querySelectorAll('[data-history-date]').forEach(node=>{node.addEventListener('pointerenter',()=>setHistoryActive(node.dataset.historyDate));node.addEventListener('pointerleave',()=>setHistoryActive(null));node.addEventListener('focusin',()=>setHistoryActive(node.dataset.historyDate));node.addEventListener('focusout',()=>setHistoryActive(null));});
  }
  function renderHistoryDetails(details, records, allRecords) {
    const disabled = !state || !summary || syncBlocked ? ' disabled' : '';
    const previousByRecord = buildHistoryPreviousRecordMap(allRecords);
    const recordCards=[...records].reverse().map(r=>{const previous=previousByRecord.get(r), change=previous?(Number(r.net_worth)||0)-(Number(previous.net_worth)||0):null, changeClass=change==null||change===0?'is-neutral':change>0?'is-positive':'is-negative', changeText=change==null?'직전 기록 없음':`${change>0?'+':''}${won(change)}`;return `<article class="wealth-history-record" data-history-date="${esc(r.date)}" tabindex="0"><div class="wealth-history-record-head"><time datetime="${esc(r.date)}">${esc(r.date)}</time><strong>${won(r.net_worth)}</strong></div><p class="wealth-history-record-change"><span>직전 기록 대비</span><strong class="wealth-history-delta ${changeClass}">${changeText}</strong></p><p class="wealth-history-record-amounts"><span class="record-asset">총자산 ${r.assets == null ? '미입력' : won(r.assets)}</span><span class="record-debt">총부채 ${r.debt == null ? '미입력' : won(r.debt)}</span></p><div class="wealth-history-record-note"><span class="record-source">구분 · ${r.source==='manual'?'직접 입력':'현재 자산 기록'}</span><span class="record-memo">메모 · ${esc(r.memo || '없음')}</span></div><div class="wealth-history-record-actions"><button type="button" class="button secondary tiny" data-history-edit="${esc(r.date)}">수정</button><button type="button" class="button text danger tiny" data-history-delete="${esc(r.date)}">삭제</button></div></article>`;}).join('');
  ...
  function buildHistoryPreviousRecordMap(records) {
    const previousByRecord = new Map(), latestByOwner = new Map();
    records.forEach(record => { const owner=record.owner||'모두'; previousByRecord.set(record,latestByOwner.get(owner)||null); latestByOwner.set(owner,record); });
  ...
  function donutGradient(items, valueField, activeKey=null) {
    let cursor=0;
    const segments=items.filter(item=>Number(item[valueField])>0).map(item=>{
      const start=cursor;
      cursor+=Number(item[valueField]);
      return `${bucketColor(item.id)}${activeKey && activeKey!==item.id ? '80' : ''} ${start}% ${Math.min(cursor,100)}%`;
    });
    return segments.length ? `conic-gradient(${segments.join(',')})` : 'var(--surface-2)';
  }
  function allocationLegend(items, valueField, showAmount=false, chartKey='') {
    return items.map(item=>`<div class="wealth-bucket-legend-row" data-bucket-chart="${chartKey}" data-bucket-key="${esc(item.id)}" tabindex="0" style="--bucket-color:${bucketColor(item.id)}"><span>${esc(item.name)}</span><strong>${Number(item[valueField]||0).toFixed(1)}%</strong>${showAmount?`<small>${won(item.value)}</small>`:''}</div>`).join('');
  }
  function renderBucketSummary() {
    ...
    const targetPanel=targetConfigured
      ? `<div class="wealth-bucket-chart-row"><div class="wealth-bucket-donut" data-bucket-chart="target" role="img" aria-label="목표 비중: ${target.map(item=>`${esc(item.name)} ${Number(item.value).toFixed(1)}%`).join(', ')}" style="background:${donutGradient(target,'value')}"><span>목표 합계</span><strong>100%</strong></div><div class="wealth-bucket-legend">${allocationLegend(target,'value',false,'target')}</div></div>`
    ...
    summaryEl.querySelectorAll('.wealth-bucket-donut').forEach(donut=>{const chart=donut.dataset.bucketChart;donut.onpointermove=e=>{const rect=donut.getBoundingClientRect(),x=e.clientX-rect.left-rect.width/2,y=e.clientY-rect.top-rect.height/2,dist=Math.hypot(x,y),outer=Math.min(rect.width,rect.height)/2;if(dist<outer*.42||dist>outer)return activate(chart,null);const pct=((Math.atan2(y,x)*180/Math.PI+450)%360)/3.6;let cursor=0;const item=chartItems[chart][0].find(value=>{const start=cursor;cursor+=Number(value[chartItems[chart][1]]||0);return pct>=start&&pct<cursor;});activate(chart,item?.id||null);};donut.onpointerleave=()=>activate(chart,null);});
  }
