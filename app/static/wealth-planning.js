...
(() => {
  ...
  function renderHistoryDetails(details, records, allRecords) {
    ...
    const recordCards=[...records].reverse().map(r=>{const previous=previousByRecord.get(r), change=previous?(Number(r.net_worth)||0)-(Number(previous.net_worth)||0):null, changeClass=change==null||change===0?'is-neutral':change>0?'is-positive':'is-negative', changeText=change==null?'직전 기록 없음':`${change>0?'+':''}${won(change)}`;return `<article class="wealth-history-record"><div class="wealth-history-record-head"><time datetime="${esc(r.date)}">${esc(r.date)}</time><strong>${won(r.net_worth)}</strong></div><p class="wealth-history-record-change"><span>직전 기록 대비</span><strong class="${changeClass}">${changeText}</strong></p><p class="wealth-history-record-amounts"><span class="record-asset">총자산 ${r.assets == null ? '미입력' : won(r.assets)}</span><span class="record-debt">총부채 ${r.debt == null ? '미입력' : won(r.debt)}</span></p><div class="wealth-history-record-note"><span class="record-source">구분 · ${r.source==='manual'?'직접 입력':'현재 자산 기록'}</span><span class="record-memo">메모 · ${esc(r.memo || '없음')}</span></div><div class="wealth-history-record-actions"><button type="button" class="button secondary tiny" data-history-edit="${esc(r.date)}">수정</button><button type="button" class="button text danger tiny" data-history-delete="${esc(r.date)}">삭제</button></div></article>`;}).join('');
