...
Only future, not-yet-realized financial income is allocatable. Current-year
realized income remains attributed to the member who already received it.
"""
...
def _baseline_money(value: object, *, allow_none: bool = False) -> int | None:
    ...
    try:
        ...
        raise FamilyFinancialIncomeAllocationError(
            "FAMILY_ALLOCATION_BASELINE_INVALID"
        ) from exc
    if not math.isfinite(number) or number < 0:
        raise FamilyFinancialIncomeAllocationError("FAMILY_ALLOCATION_BASELINE_INVALID")
    return int(round(number))
...
def _allocatable_future_income(row: dict[str, Any]) -> int | None:
    """Return future gross income that may be moved by this simulation.

    Realized YTD income is intentionally excluded. A missing future dividend
    forecast means the movable future amount is unavailable rather than zero.
    """
    components = row.get("components")
    if not isinstance(components, dict):
        raise FamilyFinancialIncomeAllocationError("FAMILY_ALLOCATION_BASELINE_INVALID")

    future_dividend = _baseline_money(
        components.get("future_months_estimated_dividend_gross_krw"),
        allow_none=True,
    )
    if future_dividend is None:
        return None

    current_month_adjustment = _baseline_money(
        components.get("current_month_remaining_dividend_gross_adjustment_krw", 0)
    )
    remaining_interest = _baseline_money(
        components.get("expected_remaining_interest_gross_krw", 0)
    )
    return int(future_dividend + current_month_adjustment + remaining_interest)


def _threshold(
    amount: int | None, before: dict[str, Any] | None
) -> dict[str, Any] | None:
...
def build_family_financial_income_allocation_simulation(
    ...
    """Apply hypothetical future-income movements to a family-risk response."""
    ...
    family_reference = baseline.get("family_reference")
    if (
        not isinstance(members, list)
        or not all(isinstance(row, dict) for row in members)
        or not isinstance(family_reference, dict)
    ):
        raise FamilyFinancialIncomeAllocationError("FAMILY_ALLOCATION_BASELINE_INVALID")
    ...
    future_available = {
        owner: _allocatable_future_income(row) for owner, row in by_owner.items()
    }
    ...
    for item in allocations:
        ...
        source = item.get("from_owner")
        destination = item.get("to_owner")
    ...
    for owner, total in outgoing.items():
        available = future_available[owner]
        if total and available is None:
            raise FamilyFinancialIncomeAllocationError(
                "FAMILY_ALLOCATION_SOURCE_FORECAST_UNAVAILABLE"
            )
        if available is not None and total > available:
            raise FamilyFinancialIncomeAllocationError(
                "FAMILY_ALLOCATION_SOURCE_AMOUNT_EXCEEDED"
            )

    rows: list[dict[str, Any]] = []
    member_projection_complete = True
    before_total = 0
    after_total = 0

    for owner, before_row in by_owner.items():
        raw_before = before_row.get("projected_gross_screening_income_krw")
        if raw_before is None:
            member_projection_complete = False
            before: int | None = None
            after: int | None = None
        else:
            before = _baseline_money(raw_before)
            after = int(before + deltas[owner])
            if after < 0:
                raise FamilyFinancialIncomeAllocationError(
                    "FAMILY_ALLOCATION_BASELINE_INVALID"
                )
        ...
        rows.append(
            {
                ...
                "allocatable_future_financial_income_krw": future_available[owner],
                "forecast_complete": before is not None,
                "thresholds": {
                    "watch": {
                        "before": (thresholds.get("watch") or {}).get(
                            "projected_gross_screening"
                        ),
                        "after": _threshold(
                            after,
                            (thresholds.get("watch") or {}).get(
                                "projected_gross_screening"
                            ),
                        ),
                    },
                    "comprehensive_tax": {
                        "before": (thresholds.get("comprehensive_tax") or {}).get(
                            "projected_gross_screening"
                        ),
                        "after": _threshold(
                            after,
                            (thresholds.get("comprehensive_tax") or {}).get(
                                "projected_gross_screening"
                            ),
                        ),
                    },
    ...
    baseline_reference_complete = family_reference.get("reference_complete") is True
    complete = member_projection_complete and baseline_reference_complete
    ...
    return {
        ...
        "family_reference": {
            "before_projected_gross_screening_income_krw": (
                before_total if complete else None
            ),
            "after_projected_gross_screening_income_krw": (
                after_total if complete else None
            ),
        ...
        "unassigned": baseline.get("unassigned") or {},
        "data_quality": {
            "screening_only": True,
            ...
            "realized_income_reallocation_allowed": False,
            "allocation_basis": "future_unrealized_financial_income_only",
            "legal_tax_notice": (
                "이 결과는 미래 금융소득 배분 가정에 따른 screening 비교이며, "
                "이미 발생한 소득의 귀속을 변경하지 않습니다. 실제 증여·명의·소득 귀속의 "
                "법률/세무 판단을 포함하지 않습니다."
            ),
        },
    }
