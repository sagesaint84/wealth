"""KB realized-record identity, verification, and duplicate classification."""
...
def kb_fingerprint(row: Mapping[str, Any], source_account_key: str, occurrence: int) -> str:
    row_hash = canonical_kb_row_hash(row)
    payload = ["kb-realized:v1", str(source_account_key), row_hash, occurrence]
    encoded = json.dumps(payload, separators=(",", ":")).encode("utf-8")
    return "kb-realized:v1:" + hashlib.sha256(encoded).hexdigest()


def classify_kb_rows(
    ...
    for row in rows:
        ...
        try:
            ...
            if row.get("canonical_hash") not in (None, row_hash):
                ...
                continue
            fingerprint = kb_fingerprint(row, source_account_key, occurrence)
        ...
        if fingerprint in existing_fingerprints:
            ...
            manual_match = False
            try:
                expected_pnl = canonical_kb_number(row.get("pnl"))
            except KBFeedError:
                output.append({"status": INVALID, "reason": "NUMERIC_PARSE_ERROR"})
                continue
            for record in existing:
...
def preview_kb_realized_selection(
    ...
    for index, item in enumerate(selected_items):
        ...
        if not isinstance(row, Mapping) or not isinstance(token, str):
            ...
            continue
        valid, reason = verify_kb_feed_row(
        ...
        if not valid:
            ...
            continue
        identity = (str(row.get("canonical_hash") or canonical_kb_row_hash(row)), int(row["source_occurrence"]))
    ...
    classified = classify_kb_rows(verified_rows, existing_records, source_account_key)
    ...
    for index, result in zip(verified_indices, classified):
        ...
        if isinstance(candidate, dict):
            candidate.update({
                ...
                "broker": str(destination_account.get("broker") or "KB증권"),
            })
        items.append(result)
