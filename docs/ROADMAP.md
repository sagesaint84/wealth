...
- 지급예정일은 공시에 명시된 경우에만 사용하며 임의 날짜를 만들지 않는다.
- 현재 연도 미래 지급월과 기존 예상월이 안전하게 매칭될 때만 확정 공시 금액을 해당 월 휴리스틱보다 우선한다.
- 기존 예상이 0인 종목은 확정 금액과 지급월이 확인되면 해당 확정 이벤트를 새 예상으로 반영한다.
- 정정공시는 공시검색에서 최신 유효 접수번호를 우선한다.
- 원문 조회/파싱 실패 시 기존 Naver/Yahoo/과거 OpenDART fallback을 유지한다.
...
- 기존 예상이 0인 경우 확정 이벤트 생성 테스트
- 기존 A-4.1 source/fallback 및 사용자별 DART credential 회귀 통과
