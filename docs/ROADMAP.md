# Wealth Roadmap

> 현재 작업 상태는 `docs/PROJECT_STATE.md`를 먼저 읽습니다.
> 이 문서는 앞으로 진행할 기능의 우선순위와 완료 기준을 기록합니다.

마지막 갱신: 2026-09-27

## 1. 현재 우선순위

### Phase 10.5B-4.3 — 금융소득 기납부 원천징수세액

상태: 진행 중

B-4.2 배당세액공제는 PR #31 merge commit `90867ae`로 완료되었습니다.

현재 increment:

- 소득세법 제76조 제3항 제4호에 따라 종합과세 금융소득에서 실제 원천징수된 국세 소득세를 명시 입력으로 반영
- 이자소득·배당소득 기납부 원천징수세액을 각각 입력하고 합계 반영
- 원천징수세액은 gross 금융소득에서 자동 추정하지 않고 실제 국세 소득세 금액만 사용자 입력
- 지방소득세 원천징수분은 이 국세 계산에서 제외
- 총 금융소득 2천만원 이하의 일반 원천징수 금융소득은 기존 Article 62 종합과세 계산에 재산입하지 않으며 해당 원천징수세액도 기납부세액으로 다시 차감하지 않음
- 결과는 배당세액공제 후 국세 참고값에서 명시 기납부 금융소득 원천징수세액을 뺀 부분 계산값이며 최종 납부·환급세액이 아님

## 2. 다음 단계

B-4.3 완료 후에는 개인지방소득세와 외국납부세액공제를 각각 공식 자료로 검증한 뒤 별도 increment로 진행합니다. 다른 소득의 기납부세액, 중간예납세액, 가산세·추가납부세액 및 다른 세액공제까지 포함한 최종 납부·환급세액 계산은 별도 범위로 유지합니다.

## 3. 금융소득/가족 세금 확장

### Phase 10.5B-1 — 가족 금융소득 위험 보기

상태: 완료 — PR #26 (`a8883c2`).

핵심 원칙:

- 본인/배우자/자녀 등 등록 소유자별 예상 금융소득
- 1,000만원 watch / 2,000만원 screening 상태는 개인별 판정
- 가족 전체 합계는 참고값으로만 제공
- 미분류 데이터는 임의 배분하지 않음

### Phase 10.5B-2 — 추가 배당/매매 What-if 확장

상태: 완료 — PR #27 merge (`404059a`).

- 추가 배당금
- 추가 이자
- 추가 해외주식 실현차익
- 국내상장 해외 ETF 과세기준금액
- 개인 vs 가족법인 비교와 연결

### Phase 10.5B-3 — 배우자/자녀 분산 시뮬레이션

상태: 완료 — PR #28 merge (`6d71cc2`).

- 자산 또는 미래 투자금 배분 시 개인별 금융소득 변화
- 단순 세금 절감 추천이 아니라 결과 비교 중심
- 증여세/명의신탁/실질귀속 등 법적 쟁점은 계산 범위에서 별도 표시
- 실제 증여세 계산은 별도 규칙 엔진 없이는 자동 확정하지 않음

### Phase 10.5B-4 — 개인 종합과세 정밀화

상태:

- first increment 완료 — PR #29 merge (`2a98442`)
- B-4.1 비교산출세액 완료 — PR #30 merge (`c65afa9`)
- B-4.2 배당세액공제 완료 — PR #31 merge (`90867ae`)
- B-4.3 금융소득 기납부 원천징수세액 진행 중

후보/후속 범위:

- 금융소득 종합과세 누진구간 — 완료
- 기본세율 비교 — 완료
- Article 62 비교산출세액 — 완료
- 배당세액공제 — 완료
- 금융소득 원천징수 기납부세액 — 진행 중
- 지방소득세
- 외국납부세액공제

원칙:

- 세법 연도별 rule module 분리
- UI에 세율 상수 하드코딩 금지
- 공식 자료 검증일 기록
- 법률/세무 최종 판단과 screening을 구분
- 출자공동사업자 배당소득의 Article 62 특수 비교는 별도 구현 전까지 fail-closed
- 기납부 금융소득 원천징수세액은 실제 국세 소득세만 명시 입력하며 지방소득세를 섞지 않음
- 다른 소득 기납부세액·중간예납·가산세·추가납부세액·지방소득세·외국납부세액공제는 각각 공식 근거 검증 후 별도 increment로 진행

## 4. 후속 세무·사회보험 영역

### 건강보험 영향

- 직장가입자/지역가입자 구분이 필요하므로 별도 모델링
- 단순 금융소득 2,000만원 기준과 동일시하지 않음
- 연도별 보험료 기준 검증 필요

### 인적공제/부양가족 영향

- 소득요건과 금융소득 과세 방식의 관계를 별도 rule로 모델링
- 배우자/자녀 분산 기능과 연결 가능

### 법인 자금 인출 확장

현재 가족법인 비교는 법인 유보 및 배당 인출 중심이다.
후보:

- 급여
- 상여
- 퇴직금
- 대여금/가지급금 위험

법률·세무 복잡도가 높으므로 단계별로 분리한다.

## 5. 알림/자동화

후보:

- 예상 금융소득 1,000만원 접근 알림
- 2,000만원 접근/초과 알림
- 확정 배당 공시 발생 알림
- 고배당기업 공시 상태 변경 알림
- Telegram 알림

원칙:

- 사용자가 명시적으로 설정한 대상만 알림
- 중복 알림 방지
- 공시 존재와 확정금액을 구분

## 6. UX 개선 backlog

완료:

- KRW 금액 입력 보조표시 `5,000,000원 · 5백만 원`

후보:

- USD 입력 천 단위 표시
- 원화 환산값 보조표시
- 배당 예상 confidence/source badge 세분화
- 공시 원문 바로가기
- 계산 근거 펼쳐보기

## 7. 장기 구조 개선 후보

### stable user id 기반 저장 경로

현재 user data path는 `data/users/<username>/...`이다.
향후 stable UUID id 기반으로 바꾸려면 다음을 반드시 별도 migration으로 처리한다.

- 기존 디렉터리 rename/move
- rollback 전략
- 중복/누락 검증
- 각 서비스의 username/path 의존성 제거
- 백업 후 migration

현재 단계에서는 경로를 임의 변경하지 않는다.

### 세법 rule source metadata

모든 세법 모듈에 다음 정보를 일관되게 보관하는 방향을 유지한다.

- rule year
- verified date
- official source URL
- 계산 범위
- not calculated 목록
- screening/legal determination 구분

## 8. 우선순위 변경 규칙

우선순위를 바꿀 때는:

1. `PROJECT_STATE.md` 현재 단계 수정
2. 이 문서 순서 수정
3. 관련 PR/Issue에 이유 기록
4. 이미 구현된 API 계약을 깨뜨리는 경우 migration/compatibility plan 기록

실제 secret, 계좌번호, 토큰, 개인 금융 데이터는 roadmap에 기록하지 않는다.
