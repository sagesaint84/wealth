...
### Phase 10.5B-4.2 — 배당세액공제

상태: 진행 중

B-4.1 금융소득 종합과세 비교산출세액은 PR #30 merge commit `c65afa9`로 완료되었습니다.

현재 increment:

- 소득세법 제56조 배당세액공제를 기존 Article 62 비교산출세액 결과에 연결
- 공식 금융소득자용 신고서 기준으로 `min(배당가산액, 종합소득산출세액 - 비교산출세액 B)` 한도 적용
- 배당가산 적격성 및 금융소득 category는 사용자 명시 입력만 사용
- 실제 기납부 원천징수세액, 다른 세액공제, 지방소득세, 외국납부세액공제는 계산하지 않음
- 결과는 최종 납부세액이 아니라 배당세액공제 후·다른 공제 전 국세 참고값

## 2. 다음 단계

B-4.2 완료 후에는 실제 기납부 원천징수세액, 개인지방소득세, 외국납부세액공제를 각각 공식 자료로 검증한 뒤 별도 increment로 진행합니다.
...
- 실제 기납부세액·지방소득세·외국납부세액공제는 각각 공식 근거 검증 후 별도 increment로 진행
