...
- 고배당기업 공식 공시 확인과 사용자의 신고 시 분리과세 신청 가정을 분리한다.
- 앱이 배당수익률 등으로 적격 여부를 임의 추정하지 않는다.
- 특례가 적용될 때만 해당 특례배당을 2,000만원 종합과세 screening 대상에서 제외한다.
- 1,000만원 제품 watch는 총 금융소득 기준을 유지한다.
- 일반 금융소득 예상과 특례 국세 screening 결과를 동시에 보여준다.
...
- 특례 시나리오가 없으면 기존 What-if 계산/서비스 호출 계약과 동일
- 신청하지 않은 경우 기존 종합과세 screening과 동일
- 공식 확인 없이 특례를 요청하면 fail-closed
- 특례금액이 추가 배당금액을 초과하면 거부
- 예상값 자체가 unavailable이면 특례 적용 후에도 임의로 0을 만들지 않음
- 고배당 특례/기존 What-if/API/static 회귀 테스트 통과
