...
현재 작업 단계는 **Phase 10.5B-4.1 — 금융소득 종합과세 비교산출세액**입니다.

현재 작업:

- branch: `phase10-5b4-1-financial-income-comparison-tax`
- base: `main`
- 상태: 구현/검증 중
- 작업 시작 기준 main: `2a98442` (PR #29 merge)

B-4.1 목표:

1. 명시 입력 금융소득 category와 출자공동사업자 배당소득을 제외한 다른 종합소득·소득공제를 이용해 소득세법 제62조 국세 비교산출세액을 screening으로 비교한다.
2. 배당가산은 사용자가 명시한 적격 배당에만 10%를 적용하며 적격성은 자동 판정하지 않는다.
3. 배당세액공제·기납부 원천징수세액·지방소득세·외국납부세액공제는 계산하지 않는다.
4. 출자공동사업자 배당소득의 제62조 특수 비교는 구현하지 않으며 해당 금액은 다른 종합소득 입력에서 제외한다.
...
- [ ] 10.5B-4.1 금융소득 종합과세 비교산출세액 — 진행 중
...
### 개인 종합과세 정밀화 — B-4 첫 increment

- 2026 소득세법 제55조의 국세 기본세율표는 연도별 rule module과 공식 source metadata로 관리한다.
- 이 increment는 사용자가 입력한 과세표준에 금융소득 포함 가정을 더한 기본세율 비교만 제공한다.
- 금융소득 종합과세의 최종 비교산식은 입력된 gross 금융소득만으로 재현하지 않는다.
- 원천징수 기납부세액, 개인지방소득세, 외국납부세액공제, 소득공제·세액공제, 배당가산 및 건강보험 영향은 자동 계산하지 않는다.
- 결과는 screening/비교 목적이며 신고 세액 또는 최종 법률·세무 판단이 아니다.
