...
현재 작업 단계는 **Phase 10.5B-4.4 — 개인지방소득세 비교산출세액**입니다.

현재 작업:

- branch: `phase10-5b4-4-local-income-tax-comparison`
- base: `main`
- 상태: 구현/검증 중
- 작업 시작 기준 main: `4281a31` (PR #32 merge)

B-4.4 목표:

1. B-4.3의 국세 Article 62 결과와 별도로 지방세법 제91조·제92조·제93조에 따른 금융소득 개인지방소득세 비교산출세액을 계산한다.
2. 지방세법 제93조 제2항의 A/B 비교식을 명시 입력 금융소득 category에 독립 적용하며 국세 결과에 단순히 10%를 곱해 대체하지 않는다.
3. 지방세법 제92조 제1항의 표준세율을 적용하고, 같은 조 제2항의 지방자치단체 조례에 따른 세율 가감은 납세지와 조례를 확인하지 않은 상태에서 자동 추정하지 않는다.
4. 지방 세액공제·감면, 배당세액공제, 특별징수 기납부세액, 가산세·추가납부세액 및 최종 납부·환급세액은 후속 increment로 분리한다.
...
- [x] 10.5B-4.3 금융소득 기납부 원천징수세액 — PR #32 merge (`4281a31`)
- [ ] 10.5B-4.4 개인지방소득세 비교산출세액 — 진행 중
...
- B-4.4는 지방세법 제91조의 과세표준 연결, 제92조 제1항 표준세율, 제93조 제2항 금융소득 A/B 비교를 독립 계산한다.
- B-4.4는 지방자치단체 조례에 따른 표준세율 가감, 지방 세액공제·감면, 배당세액공제, 특별징수 기납부세액 및 최종 지방소득세 납부·환급액을 계산하지 않는다.
- 현재 결과에는 다른 국세 세액공제·감면, 다른 소득 국세 기납부세액, 중간예납, 가산세·추가납부세액, 외국납부세액공제 및 건강보험 영향이 포함되지 않는다.
- 국세 기납부 금융소득 원천징수세액 반영 후의 signed balance와 지방세 Article 93 비교산출세액은 모두 screening/계산보조용 부분값이며 신고상 최종 납부 또는 환급세액이 아니다.
