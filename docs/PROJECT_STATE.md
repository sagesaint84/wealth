...
현재 작업 단계는 **Phase 10.5B-4.2 — 배당세액공제**입니다.

현재 작업:

- branch: `phase10-5b4-2-dividend-tax-credit`
- base: `main`
- 상태: 구현/검증 중
- 작업 시작 기준 main: `c65afa9` (PR #30 merge)

B-4.2 목표:

1. B-4.1의 소득세법 제62조 비교산출세액 결과에 소득세법 제56조 배당세액공제를 연결한다.
2. 공식 금융소득자용 신고서의 계산식에 따라 배당세액공제는 `배당가산액`과 `종합소득산출세액 - 비교산출세액 B` 중 작은 금액으로 제한한다.
3. 배당가산 적격성 및 금융소득 category는 사용자가 명시한 입력만 사용하며 자동 법적 분류를 하지 않는다.
4. 실제 기납부 원천징수세액, 다른 세액공제·감면, 개인지방소득세, 외국납부세액공제는 계산하지 않으며 결과를 최종 납부세액으로 표시하지 않는다.
...
- B-4.2 결과는 배당세액공제 후·다른 공제 전 국세 참고값이며 실제 기납부 원천징수세액, 다른 세액공제·감면, 개인지방소득세, 외국납부세액공제 및 건강보험 영향은 계산하지 않는다.
- 결과는 screening/계산보조 목적이며 신고 세액 또는 최종 법률·세무 판단이 아니다.
