...
현재 작업 단계는 **Phase 10.5B-4.3 — 금융소득 기납부 원천징수세액**입니다.

현재 작업:

- branch: `phase10-5b4-3-prepaid-financial-withholding`
- base: `main`
- 상태: 구현/검증 중
- 작업 시작 기준 main: `90867ae` (PR #31 merge)

B-4.3 목표:

1. B-4.2의 Article 62 비교산출세액 및 배당세액공제 결과에 소득세법 제76조 제3항 제4호의 기납부 원천징수세액을 연결한다.
2. 이자소득·배당소득에서 실제 원천징수된 **국세 소득세** 금액만 사용자 명시 입력으로 받으며 gross 금융소득에서 자동 추정하지 않는다.
3. 지방소득세 원천징수분은 국세 기납부세액 입력에서 제외하고, 총 금융소득 2천만원 이하의 일반 원천징수 금융소득은 Article 62 종합과세 계산에 재산입하거나 다시 차감하지 않는다.
4. 다른 세액공제·감면, 다른 소득의 기납부세액, 중간예납세액, 가산세·추가납부세액, 개인지방소득세, 외국납부세액공제는 계산하지 않으며 결과를 최종 납부·환급세액으로 표시하지 않는다.
...
- [x] 10.5B-3 배우자/자녀 분산 시뮬레이션 — PR #28 merge (`6d71cc2`)
- [x] 10.5B-4 개인 종합과세 정밀화 first increment — PR #29 merge (`2a98442`)
- [x] 10.5B-4.1 금융소득 종합과세 비교산출세액 — PR #30 merge (`c65afa9`)
- [x] 10.5B-4.2 배당세액공제 — PR #31 merge (`90867ae`)
- [ ] 10.5B-4.3 금융소득 기납부 원천징수세액 — 진행 중
...
- B-4 first increment는 2026 소득세법 제55조 기본세율과 사용자가 명시한 과세표준 전/후 비교를 제공한다.
- B-4.1은 명시 입력 금융소득 category, 출자공동사업자 배당소득을 제외한 다른 종합소득, 소득공제를 이용해 소득세법 제62조 A/B 비교산출세액을 계산한다.
- B-4.1의 배당가산은 사용자가 명시한 Gross-Up 적격 배당 중 종합과세기준금액 초과분에만 10%를 적용한다.
- B-4.1은 출자공동사업자 배당소득의 제62조 특수 비교를 구현하지 않으며 해당 금액은 다른 종합소득 입력에서 제외한다.
- B-4.2는 배당가산액과 `종합소득산출세액 - 비교산출세액 B` 중 작은 금액을 소득세법 제56조 배당세액공제로 계산한다.
- B-4.3은 종합과세 금융소득에 대해 실제 원천징수된 이자·배당 국세 소득세를 명시 입력으로 받아 배당세액공제 후 국세 참고값에서 차감한다.
- B-4.3은 지방소득세 원천징수액을 포함하지 않고, gross 금융소득만으로 실제 기납부세액을 추정하지 않는다.
- 현재 결과에는 다른 세액공제·감면, 다른 소득 기납부세액, 중간예납, 가산세·추가납부세액, 개인지방소득세, 외국납부세액공제 및 건강보험 영향이 포함되지 않는다.
- 기납부 금융소득 원천징수세액 반영 후의 signed balance도 screening/계산보조용 부분값이며 신고상 최종 납부 또는 환급세액이 아니다.
