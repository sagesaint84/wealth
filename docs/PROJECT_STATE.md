...
현재 작업 단계는 **Phase 10.5B-4 — 개인 종합과세 정밀화**입니다.

현재 작업:

- branch: `phase10-5b4-personal-comprehensive-tax`
- base: `main`
- 상태: 구현/검증 중
- 작업 시작 기준 main: `6d71cc2` (PR #28 merge)

B-4 첫 increment 목표:

1. 사용자가 명시적으로 입력한 과세표준과 금융소득 포함 가정을 기준으로 2026 국세 기본세율의 전/후 비교를 제공한다.
2. 금융소득 2,000만원 screening 상태는 기존 개인별 threshold 계약(`at_or_above` / `exceeded`)을 재사용한다.
3. 실제 금융소득 종합과세 비교산식, 소득공제·세액공제, 배당가산, 기납부 원천징수세액, 개인지방소득세, 외국납부세액공제는 계산하지 않는다.
4. 입력과 응답은 request 단위의 stateless 비교이며 portfolio·배당기록·사용자 설정을 변경하지 않는다.
