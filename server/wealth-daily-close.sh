...
from app.main import (
    ...
    refresh_prices,
    dashboard,
    auto_save_all_owner_snapshots,
    save_all_owner_net_worth_snapshots,
)
...
async def run():
    ...
    # 3. 최신 시세가 반영된 dashboard 구성 (조회 자체는 기록을 만들지 않음)
    data = await dashboard(
        make_request("/api/dashboard", "GET"),
        record_snapshots=False,
    )

    # 4. 주식기록 + 순자산기록을 모든 owner에 대해 21시 마감값으로 저장
    ...
    net_record = next(
        (
            ...
            if record.get("date") == today and record.get("owner") == "모두"
        ),
        None,
    )
...
print(
    ...
    + " stock_record_count=" + str(d.get("stock_record_count"))
    + " net_record=" + str(d.get("net_record_saved"))
    + " net_record_count=" + str(d.get("net_record_count"))
